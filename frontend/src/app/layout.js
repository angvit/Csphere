    ...
    >
      <body className="relative font-custom">
        <header
          ...
          // h-16: Sets a fixed height (approx 64px, adjust as needed)
          // bg-gray-300: Background color
          // w-full: Full width
          // flex, items-center, justify-between: Flexbox layout
          // px-4: Base horizontal padding for mobile
          // sm:px-6 lg:px-8: Increase padding on larger screens (adjust values as needed)
        ...
        >
          ...
          <div className="flex items-center space-x-4 md:space-x-8">
            ...
            <Link href="/" className="flex-shrink-0">
              ...
              {/* SVG Logo: Control size with Tailwind, remove fixed width/height */}
              {/* h-10: Set height, w-auto keeps aspect ratio. Adjust height as needed */}
              {/* viewBox needs to be appropriate for the path data */}
              <svg
                // viewBox="0 0 400 100" // Original - Too small vertically
                ...
                // className controls the display size, viewBox controls the internal coordinate system & clipping box
              ...
              >
                {/* --- SVG Paths --- */}
                {/* Coordinates should now be fully visible within the new viewBox */}
            ...
            {/* Desktop Navigation Links: Hidden on mobile, shown md breakpoint and up */}
            <nav className="hidden md:flex md:items-center md:space-x-6">
              {/* Use text-sm or text-base for font size */}
              <a
                href="#"
          ...
          {/* Right side: Buttons and Mobile Menu Trigger */}
          <div className="flex items-center space-x-3">
            {" "}
            {/* Adjust spacing */}
            {/* Buttons: Use only Tailwind classes */}
            {/* Hide Contact button on small screens as an example */}
            <button
              className="hidden sm:inline-block bg-[#202A29] text-white px-4 py-2 rounded-lg hover:bg-[#2D3B3A] text-sm font-medium"
              // Removed inline style
            >
              Contact
            </button>
            <button
              className="bg-[#E0E5E4] text-[#202A29] px-4 py-2 rounded-lg hover:bg-[#CCD3D2] text-sm font-medium"
              // Removed inline style
            >
              {/* Wrap link content directly */}
              <Link href="/login">Log in</Link>
            </button>
            {/* Mobile Menu Button: Shown only on mobile (hidden md and up) */}
